...
6. [⚠️ IMPORTANTE: Workflow Después de Cada Cambio](#️-importante-workflow-después-de-cada-cambio)
...
## ⚠️ IMPORTANTE: Workflow Después de Cada Cambio

**🚨 SIEMPRE que hagas cambios en tu portfolio (agregar fotos, videos, cambiar textos, etc.), DEBES ejecutar estos 4 comandos en orden:**

```bash
# 1. Agregar los cambios al staging
git add .

# 2. Crear un commit con descripción de los cambios
git commit -m "Descripción de los cambios"

# 3. Subir los cambios a GitHub
git push

# 4. Desplegar la nueva versión en GitHub Pages
npm run deploy
```

### 📋 Ejemplo Completo

Si agregaste un nuevo videoclip:

```bash
git add .
git commit -m "Añadir videoclip Flame yrn - Benzema"
git push
npm run deploy
```

Si agregaste nuevas fotos de retrato:

```bash
git add .
git commit -m "Añadir 5 nuevas fotos de retrato"
git push
npm run deploy
```
...
- `git add .` → Instantáneo
- `git commit` → Instantáneo
- `git push` → 5-10 segundos
- `npm run deploy` → 30-60 segundos

**Total**: ~1-2 minutos para que tus cambios estén en vivo en Internet.
...
Esto significa que tu sitio está actualizado en **https://romppao.es/**
